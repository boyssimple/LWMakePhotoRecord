...
@interface LWVideoRecordManager ()<AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureAudioDataOutputSampleBufferDelegate, CAAnimationDelegate,AVCapturePhotoCaptureDelegate> {
...
-(void)startPicture{
    ...
    [self.photoSettings setFlashMode:AVCaptureFlashModeAuto];
    if (!self.previewLayer || !self.photoOutput || !self.photoSettings) {
        return;
    }
    if (self.recordSession.isRunning == YES) {
...
- (AVCaptureSession *)recordSession {
    if (_recordSession == nil) {
        ...
        //添加照片输出
        if ([_recordSession canAddOutput:self.photoOutput]) {
            [_recordSession addOutput:self.photoOutput];
        }
        //设置视频录制的方向
...
- (void)captureOutput:(AVCapturePhotoOutput *)output didFinishProcessingPhoto:(AVCapturePhoto *)photo error:(NSError *)error{
    if (!error) {
        ...
        NSLog(@"拍照完成");
        if ([self.delegate respondsToSelector:@selector(makePictureFinished:)]) {
            [self.delegate makePictureFinished:image];
        }
